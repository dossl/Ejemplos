IF EXISTS (SELECT name FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[Usp_CW_Sel_Tesoreria_met_tran_cxc]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1)
                DROP PROCEDURE [dbo].[Usp_CW_Sel_Tesoreria_met_tran_cxc]
GO

IF NOT EXISTS (SELECT name FROM sys.types WHERE is_table_type = 1 AND name = 'cur_tedetatran')
	CREATE TYPE cur_tedetatran AS TABLE(   id INT ,--IDENTITY(1,1) Primary Key,
									 cdetipotra varchar (100), cdetransac varchar(100),cdocurefer varchar (25),nmontotran numeric (15,2),
									 namortizac numeric (15,2),	 nintereses numeric(15,2), nmoratorio numeric(15,2),ccuentacon varchar(35),
									 ccodprovee varchar(4), ctipotrans varchar(1), cidtransac varchar(6), cobservaci varchar(250),
									 cutiajupag varchar (1) default 'N',cllave varchar(25)
									
									)

GO
 
	

---- ===================================================================================== ----
-- Este script debe ser ejecutado en la base de datos del cliente
---- ===================================================================================== ----
-- Módulo				:  Tesoreria
-- Sub-módulo			: Procesos
-- Opción				: Transaccion de cajas
-- Cliente				: Quarzo
---- ===================================================================================== ----
-- Detalle				: Procedimiento almacenado que anula un recibo.
--
-- Autor.....			: Deivis Leal (@SanPanda)- para Ingenius
-- Creación..			: 
-- Modificado			: 27/06/2017
-- Modificado por		: 
-- Parámetros			: 
---- ===================================================================================== ----

CREATE PROCEDURE [dbo].[Usp_CW_Sel_Tesoreria_met_tran_cxc]
(
@pccuentacaj varchar (35),
@pcidcliente varchar (10),
@pctipasient varchar (3),
@pcnumasient varchar (30),
@pccodigocaj varchar (4),
@pdfechatran datetime,
@pnnumereci numeric (18,2),
@pcidconsecu varchar (20),
@vpbc_codusuario varchar (40),
@idCursor int,--para saber que fila es la que estoy analizando
@cur_tedetatran dbo.cur_tedetatran READONLY,
@nIdLogExcAG int output,
@cMensajeExc varchar(2000) output

)		

AS 
BEGIN
DECLARE

--VARIABLES EXCEPCIONES:
	@vlci_errorNumber int, @vlci_errorSeverity int, @vlci_errorState int, @vlcc_errorProcedure varchar(128),
	@vlci_errorLine int, @vlcc_errorMessage varchar(4000), @vlcd_fechaActual datetime, @TranName VARCHAR(200), @cantReg int
	,@ValorRetorno varchar(5)
--VARIABLES LOCALES

  --declare
	  --@vlcc_nconsecuti varchar(200)

	  --ASIGNANDO VALOR POR DEFECTO A LAS VAR parametros

	--ASIGNANDO VALOR POR DEFECTO A LAS VAR EXCEPCIONES
	SET @nIdLogExcAG = 0
	SET @vlci_errorNumber    = 0
	SET @vlci_errorSeverity  = 0
	SET @vlci_errorState     = 0
	SET @vlcc_errorProcedure = ''
	SET @cMensajeExc = ''
	SET @vlci_errorLine      = 0
	SET @cantReg			 = 0
	SET @vlcc_errorMessage   = ''	
	set @vlcd_fechaActual = getdate()
	SET @ValorRetorno = 'False'
	SELECT @TranName = 'Usp_CW_Sel_Tesoreria_met_tran_cxc'

	
	DECLARE @open_tran int = @@TRANCOUNT

	
		BEGIN TRY
		IF @open_tran > 0
			SAVE TRANSACTION @TranName
		ELSE
			BEGIN TRANSACTION @TranName
	BEGIN	

	---------Met Proceso----------------
	BEGIN
	DECLARE @nconstrans numeric (18,0),@prontopago numeric (18,2), @cdocurefer varchar (35), @cidtransac varchar (6), @nmontotran numeric (18,2),
	@namorcalcu numeric (18,2), @nintecalcu  numeric (18,2), @nmoracalcu  numeric (18,2)

	
	SELECT @cdocurefer= cdocurefer, @cidtransac= cidtransac, @nmontotran= nmontotran FROM @cur_tedetatran WHERE id=@idCursor
	 -------------------------------------------pagar--------------------------------------------
	 EXEC Usp_CW_Sel_Tesoreria_met_pagar  @pcidcliente, @cdocurefer, @cidtransac, @nmontotran, @pctipasient, @pcnumasient, @pccodigocaj,
	                                      @pnnumereci, @pcidconsecu, @pdfechatran, @vpbc_codusuario, @nconstrans output,@prontopago output,
										  @namorcalcu output, @nintecalcu output, @nmoracalcu output, @nIdLogExcAG output, @cMensajeExc output
	  IF @nIdLogExcAG=-1
			   RETURN
	 ----------------------------------------FIN pagar---------------------------------------------------------------------------------------
	
	DECLARE @vlcc_namortizac numeric (18,2), @vlcc_nintereses numeric (18,2), @vlcc_nmoratorio numeric (18,2),@vlcc_nconstrans numeric (18,0),
	@vlcc_cidtransac varchar (6), @vlcc_ctipotrans varchar (3), @vlcc_cdocurefer varchar (35), @vlcc_nmontotran numeric (18,2),
	@vlcc_ccodprovee varchar (35), @vlcc_ccuentacon varchar (35)

	
	SET @vlcc_namortizac = @namorcalcu
	SET @vlcc_nintereses = @nintecalcu
	SET @vlcc_nmoratorio = @nmoracalcu
	SET @vlcc_nconstrans = @nconstrans
	SET @vlcc_ctipotrans = 'C'
	SET @vlcc_ccodprovee = NULL
	SET @vlcc_ccuentacon = NULL
	
	SELECT @vlcc_cidtransac= cidtransac, @vlcc_cdocurefer=cdocurefer, @vlcc_nmontotran= nmontotran  FROM @cur_tedetatran WHERE id=@idCursor
	
	  INSERT INTO tedetatran (nidrecibos, ctipotrans, cidtransac, cdocurefer, 
	  nmontotran, namortizac, nintereses, nmoratorio, ccodprovee, ccuentacon, nconstrans) 
	  VALUES (ident_current('teencatran'),@vlcc_ctipotrans, @vlcc_cidtransac, @vlcc_cdocurefer, 
	  @vlcc_nmontotran, @vlcc_namortizac, @vlcc_nintereses, @vlcc_nmoratorio, @vlcc_ccodprovee, 
	  @vlcc_ccuentacon, @vlcc_nconstrans)

	
	
	
	END
	
	
	-------Fin Met Proceso-----------
	
	SET @ValorRetorno = 'True'
	END
	IF @open_tran = 0
	COMMIT TRANSACTION @TranName
	END TRY
	BEGIN CATCH
		IF Xact_state() <> -1 --SE HACE ROLLBACK EN TODO CASO
			ROLLBACK TRANSACTION @TranName
		ELSE IF @open_tran = 0 --SE HACE ROLLBACK SOLO SI ESTOY EN LA TRANSACCION
			ROLLBACK TRANSACTION @TranName
			
		IF Xact_state() <> -1 OR @open_tran = 0 
			EXEC Usp_cw_Ins_RegistExcepcion @vlci_errorNumber, @vlci_errorSeverity, @vlci_errorState,  @vlcc_errorProcedure, 
											@vlci_errorLine,   @vlcc_errorMessage,  @vlcd_fechaActual, @cMensajeExc,     
											@nIdLogExcAG OUT
		
		SET @ValorRetorno = 'False'
		SET @nIdLogExcAG = -1	
		SELECT @vlci_errorNumber = ERROR_NUMBER(),	@vlci_errorSeverity = ERROR_SEVERITY(), @vlci_errorState = ERROR_STATE()
	   ,@vlcc_errorProcedure  = ERROR_PROCEDURE(), @vlci_errorLine = ERROR_LINE(), @vlcc_errorMessage = ERROR_MESSAGE()
	   if(@cMensajeExc is null or @cMensajeExc = '')
	   begin
			SET @cMensajeExc = 'No fue Realizar el proceso.' + @vlcc_errorMessage			
		end
	
	END CATCH  
END--Procedure
GO

-- ===================================================================================== ----
-- Detalle		: Script para actualizar el diccionario DML.
-- Autor		: Victor M. Pelegrino - para Quarzo Innovación
-- Creacion		: 07/02/2017
-- Modificado por : 
-- Modificado   : 
-- ===================================================================================== ----

IF EXISTS (SELECT name FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Usp_Ins_General_IngresaDML_Filter]'))
    EXEC Usp_Ins_General_IngresaDML_Filter '[dbo].[Usp_CW_Sel_Tesoreria_met_tran_cxc]', 'Deivis', '2017/06/27', '', '', 
		'Procedimiento almacenado para ejecutar el proceso transacciones de cajas', 13
GO


